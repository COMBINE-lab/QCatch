...
def save_results(args, version, intermediate_result, valid_bcs):
    ...
    # AnnData >=0.13 also exposes X under the None key. Preserve it while
    # sorting named layers, without comparing None to strings.
    args.input.mtx_data.layers = OrderedDict(
        sorted(args.input.mtx_data.layers.items(), key=lambda item: (item[0] is not None, item[0] or ""))
    )
    ...
    if args.save_filtered_h5ad:
        ...
        # Sort layers alphabetically for deterministic h5ad output
        filter_mtx_data.layers = OrderedDict(
            sorted(filter_mtx_data.layers.items(), key=lambda item: (item[0] is not None, item[0] or ""))
        )
